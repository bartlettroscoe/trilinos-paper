...
\subsection{Thyra and Stratimikos}
\label{sec:thyra}
\label{sec:stratimikos}

The Thyra\cite{???} package contains a set of abstract polymorphic interfaces and supporting software for building abstract numerical algorithms.
Abstract numerical algorithms (ANAs) only require basic vector and multi-vector operations, linear operator applications, and functions for computing vectors and linear operators and don't require any knowledge or low-level access to the elements of these objects.
Examples of Trilinos packages that implement ANAs using Thyra include Belos, Anasazi, NOX, Tempus, ROL, and Piro.
As a result, the implemented ANAs can be efficiently applied to any problem domain and can exploit any problem structure.

The Thyra package is composed the following subpacka:

* \text{Thyra



for vectors, vector spaces, and linear operators.
I
